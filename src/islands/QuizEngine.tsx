...
export default function QuizEngine({ questions, types, groupColors, groupInfo, dimensionLabels, lang, labels }: Props) {
  ...
  if (screen === 'landing') {
    return (
      <div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:72px 24px 60px;">
  ...
  if (screen === 'quiz') {
    ...
    return (
      <div style="max-width:640px;margin:0 auto;padding:24px 24px 60px;">
        ...
        {/* Question text */}
        <div style="font-size:22px;font-weight:700;line-height:1.4;margin-bottom:28px;text-align:center;">
  ...
  if (screen === 'result' && resultCode && types[resultCode]) {
    ...
    return (
      <div style="max-width:640px;margin:0 auto;padding:24px 16px 60px;">
        {/* Hero card */}
        <div style={`background:var(--color-card-bg);border:1px solid var(--color-card-border);border-radius:16px;padding:32px 24px;margin-bottom:12px;text-align:center;position:relative;overflow:hidden;`}>
          ...
          <div style={`font-size:42px;font-weight:900;line-height:1.1;margin-bottom:8px;letter-spacing:-0.02em;color:${gc};`}>
        ...
        {/* Summary card */}
        <div style="background:var(--color-card-bg);border:1px solid var(--color-card-border);border-radius:16px;padding:32px 24px;margin-bottom:12px;">
        ...
        <div style="background:var(--color-card-bg);border:1px solid var(--color-card-border);border-radius:16px;padding:32px 24px;text-align:center;">
          <button class="btn-clip btn-red" onClick={startQuiz} style="margin:6px;padding:12px 24px;">
            {labels.resultRetry}
          </button>
          <br />
          <a
        ...
        {showAllTypes && (
          <div
            style="position:fixed;inset:0;z-index:300;background:rgba(0,0,0,0.85);overflow-y:auto;padding:80px 24px 40px;"
            onClick={(e) => { if (e.target === e.currentTarget) setShowAllTypes(false); }}
          >
            <button
              onClick={() => setShowAllTypes(false)}
              style="position:fixed;top:20px;right:24px;z-index:310;background:none;border:none;cursor:pointer;color:#fff;font-size:28px;"
            >
              &times;
            </button>
            <div style="text-align:center;font-size:28px;font-weight:800;margin-bottom:32px;letter-spacing:-0.02em;color:#fff;">
              {labels.allTypesTitle}
            </div>
            <div style="max-width:900px;margin:0 auto;display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:16px;">
              {Object.values(types).map((t) => (
                <div
                  key={t.code}
                  onClick={() => viewTypeResult(t.code)}
                  style={`background:var(--color-card-bg);border:1px solid var(--color-card-border);border-radius:12px;padding:16px;text-align:center;cursor:pointer;transition:transform 0.2s;`}
                >
